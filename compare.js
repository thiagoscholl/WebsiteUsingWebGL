...
async function main() {
  ...
  void main() {
    ...
    gl_Position = u_projection * u_view * worldPosition + vec4(u_translation, 0);
  ...
  function getGeometriesExtents(geometries) {
    ...
    const linePosition = Math.random() > 0.5 ? 1 * 1 : -1 * 1;
  ...
  const cameraTarget = [linePosition, 0, 0];
  ...
  function render(time) {
    ...
    function animacaoCurva() {
      const x = a * Math.cos(t);
      const y = b * Math.sin(t);
      const z = c * Math.sin(t);
